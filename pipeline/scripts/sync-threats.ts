...
async function main() {
  ...
  try {
    ...
    // Self-healing: upsert any registry stacks missing from the DB so that
    // threat_stack FK constraints are always satisfiable.
    const missingSlugs = STACK_REGISTRY.map(c => c.slug).filter(s => !stackIdMap.has(s));
    if (missingSlugs.length > 0) {
      console.log(`  → upserting ${missingSlugs.length} missing stacks...`);
      await Promise.all(
        STACK_REGISTRY.filter(cfg => missingSlugs.includes(cfg.slug)).map(cfg =>
          db.insert(stack).values({
            slug:          cfg.slug,
            name:          cfg.name,
            sortOrder:     cfg.sortOrder,
            catalogStatus: cfg.catalogStatus,
            ecosystem:     cfg.ecosystem ?? null,
            nvdKeywords:   cfg.nvdKeywords ?? [],
            osvEcosystem:  cfg.osvEcosystem ?? null,
          }).onConflictDoUpdate({
            target: stack.slug,
      ...
      const refreshed = await db.select().from(stack);
      refreshed.forEach(s => stackIdMap.set(s.slug, s.id));
      console.log(`  → stackIdMap rebuilt: ${stackIdMap.size} entries`);
    }
    ...
    for (const t of toInsert) {
      try {
        const written = await upsertThreat(t);
        if (written) {
          updatedIds.push(t.publicId);
          counts[sourceKey(t.source)].upserted++;
        } else {
          counts[sourceKey(t.source)].skipped++;
        }
      } catch (e) {
        console.error(`Failed upserting ${t.publicId}:`, e);
        counts[sourceKey(t.source)].errors++;
      }
    }

    // ── Phase 7: Upsert threat_stack rows ─────────────────────────────
    // Only link threats that were actually written to the threat table.
    // upsertThreat silently skips old CVEs (pre-PUBLISHED_SINCE) so we must
    // guard here — otherwise threat_stack gets an FK violation for the parent
    // row that was never inserted.
    console.log("Phase 7: upserting threat_stack rows...");
    const upsertedSet = new Set(updatedIds);
    for (const t of toInsert) {
      if (!upsertedSet.has(t.publicId)) continue;
