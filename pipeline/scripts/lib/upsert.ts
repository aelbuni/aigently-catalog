...
const PUBLISHED_SINCE = new Date(process.env.PUBLISHED_SINCE ?? "2023-01-01");

export async function upsertThreat(t: NormalisedThreat): Promise<boolean> {
  // Skip old CVEs that are no longer actionable, but always keep actively
  // exploited threats (CISA KEV entries) regardless of age.
  if (t.publishedAt && t.publishedAt < PUBLISHED_SINCE && !t.isActivelyExploited) {
    return false;
  }
