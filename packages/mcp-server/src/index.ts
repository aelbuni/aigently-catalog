...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          intent: {
            type: "string",
            description:
              "What you are building — used to auto-select the rule type. " +
              "E.g. 'user auth with JWT', 'upgrade vulnerable npm packages'.",
          },
          rule_type: {
            type: "string",
            enum: ["all", "patterns", "deps"],
            description:
              "Which guardrail type to return: " +
              "'patterns' = ALWAYS/NEVER safe-coding directives; " +
              "'deps' = dependency vulnerability advisories; " +
              "'all' = both (default). Inferred from intent when omitted.",
          },
          target_ide: {
            type: "string",
            description:
    ...
    {
      name: "list_layers",
      description: "List the security protection layer taxonomy (informational only — layers are now tags for browsing, not composition axes). Use compose_guardrail with rule_type instead.",
      inputSchema: { type: "object", properties: {}, required: [] },
    },
    {
      name: "get_manifest",
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  const result = await (async () => {
    switch (name) {
      ...
      case "list_layers":          return { layers: LAYER_TAXONOMY, total: LAYER_TAXONOMY.length, tip: "Layers are taxonomy tags for browsing. Use compose_guardrail with rule_type: 'patterns' | 'deps' | 'all' to get guardrail content." };
